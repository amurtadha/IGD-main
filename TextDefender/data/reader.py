...
class ClassificationReader(BaseReader):
    ...
    def read_from_file(self, file_path, split='train', args=None):
        temp_label_set = set()
        instances = []
        with open(f'{file_path}/{split}.txt', 'r', encoding='utf-8') as fin:
            ...
            for line in fin:
                ...
                if len(ss) == 3:
                    ...
                    if args and args.training_type == 'ours' and args.dataset_name in ['imdb', 'sst2']:
                        ss[1] ='0'  if ss[1] =='1' else '1'
...
class ClassificationReaderSpacy(BaseReader):
    def __init__(self, model_type='lstm', max_seq_len=128):
        ...
        self.max_seq_len = max_seq_len
        self.label_map = dict()
    def read_from_json(self, file_path, split='train'):
        """Reads a json list file."""
        temp_label_set = set()
        instances = []

        data = json.load( open(file_path, "r"))
        # label = json.load( open(input_file.replace('test', 'labels').replace('train', 'labels').replace('dev', 'labels'), "r"))

        for i in range(len(data)):
            idx = f'{split}-{i}'
            temp_label_set.add(data[i]['label'])
            instances.append(InputInstance(idx, data[i]['text'], None, data[i]['label']))
        for (i, x) in enumerate(sorted(temp_label_set)):
            self.label_map[x] = i
        return instances
    def read_from_file(self, file_path, split='train'):
