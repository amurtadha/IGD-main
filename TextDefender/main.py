...
import sys
sys.path.append("/workspace/IGD/baselines/tmd-main/src/")
from models import AutoLanguageModel, AutoGenerativeModel
from MyModel import  MyIGD
...
class AttackBenchmarkTask(object):
    def __init__(self, args: ProgramArgs):
        ...
        if  args.training_type== 'ours':
            self.train_raw, self.eval_raw, self.test_raw = auto_create(
                f'{args.model_type}/{args.dataset_name}_raw_datasets_ours', lambda: self._build_raw_dataset(args),
                True, path=args.cache_path
            )
            self.train_dataset, self.eval_dataset, self.test_dataset = auto_create(
                f'{args.model_type}/{args.dataset_name}_tokenized_datasets_ours',
                lambda: self._build_tokenized_dataset(args),
                True, path=args.cache_path
            )

        elif args.use_dev_aug == 'False':
    ...
    def evaluate(self, args: ProgramArgs, is_training: bool = False) -> Metric:
        if is_training:
            ...
            if args.training_type == 'ours':
                ...
                if args.inactivate_data_loss:
                    best_model_path = '{}{}_{}_final_without_new_{}_{}.bm'.format(args.states, args.dataset_name,
                                                                                  args.model_type, args.n_candidates,
                                                                                  args.m)
                else:
                    best_model_path = '{}{}_{}_final_new_{}_{}.bm'.format(args.states, args.dataset_name,
                                                                          args.model_type, args.n_candidates, args.m)



                self.model.load_state_dict(torch.load(best_model_path), strict=False)
            elif args.training_type not in ['none', 'tmd']:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # elif args.training_type == 'ours':
            #     assert isinstance(batch[0], torch.Tensor)
            #     batch = tuple(t.to(args.device) for t in batch)
            #     golds = batch[3]
            #     inputs = convert_batch_to_bert_input_dict(batch, args.model_type)
            #     logits = self.model.forward([inputs['input_ids'],inputs['token_type_ids'],inputs['attention_mask']])[0]
    ...
    def attack(self, args: ProgramArgs):
        print( args.training_type, args.dataset_name, args.inactivate_data_loss)
        # exit()
        if args.training_type =='ours':
            # dta_dic={'agnews':'AG', 'imdb':'IMDB', 'yelppolarity':'YELP-2'}
            if args.inactivate_data_loss:
                best_model_path = '{}{}_{}_final_without_new_{}_{}.bm'.format(args.states, args.dataset_name,args.model_type, args.n_candidates,args.m)
                # best_model_path = '{}{}_{}_final_without_new_{}_{}.bm'.format(args.states, args.dataset_name,args.model_type, args.n_candidates,args.m)
            else:
                # best_model_path = '/workspace/IGD/state_dict/{}_{}_final_new_{}_10_s_{}.bm'.format( dta_dic.get(args.dataset_name), args.model_type,args.n_candidates,args.m)
                # best_model_path = '/workspace/IGD/state_dict/{}_{}_final_new_{}_{}_{}.bm'.format( dta_dic.get(args.dataset_name), args.model_type,args.n_candidates, '_without_lm' if args.inactivate_lm_loss else '')
                best_model_path = '{}{}_{}_final_new_{}_{}.bm'.format(args.states, args.dataset_name, args.model_type,args.n_candidates,args.m)
        ...
        attacker_log_path = os.path.join(f"{args.workspace}/log/{args.dataset_name}_{args.model_type}",
                                         attacker_log_path)
        # print(os.path.join(attacker_log_path,
        #                    f'{args.attack_method}_{args.neighbour_vocab_size}_{args.modify_ratio}_{args.n_candidates}_{args.inactivate_data_loss}.txt'))
        # print('dddddddddddddddddddddddddddddddddddddddddddddone')
        # exit()

        attacker_log_manager = AttackLogManager()
        # attacker_log_manager.enable_stdout()
        if args.training_type in ['ours']:
            # attacker_log_manager.add_output_file(os.path.join(attacker_log_path, f'{args.attack_method}_{args.neighbour_vocab_size}_{args.modify_ratio}_{args.n_candidates}_{args.inactivate_data_loss}.txt'))
            attacker_log_manager.add_output_file(os.path.join(attacker_log_path,'{}_{}_{}_{}_{}{}{}.txt'.format(args.attack_method,args.neighbour_vocab_size,args.modify_ratio,args.n_candidates,args.inactivate_data_loss,'_without_lm' if args.inactivate_lm_loss else '', args.m)))
        else:
            # attacker_log_manager.add_output_file(os.path.join(attacker_log_path, f'{args.attack_method}_{args.neighbour_vocab_size}_{args.modify_ratio}_{args.n_candidates}.txt'))
            attacker_log_manager.add_output_file(os.path.join(attacker_log_path, f'{args.attack_method}_{args.neighbour_vocab_size}_{args.modify_ratio}.txt'))
    ...
    def _build_model(self, args: ProgramArgs):
        ...
        if args.model_type == 'lstm':
            ...
            args.lebel_dim={'agnews':4,'imdb':2, 'yelppolarity':2, 'sst-2':2 }.get(args.dataset_name)
            ...
            bert_vocab = get_bert_vocab(args)
            ...
            roberta_vocab = get_roberta_vocab(args)
            hull = DecayAlphaHull.build(
                alpha=args.dir_alpha,
                decay=args.dir_decay,
                nbr_file=args.nbr_file,
            ...
            xlnet_vocab = get_xlnet_vocab(args)
            hull = DecayAlphaHull.build(
                alpha=args.dir_alpha,
                decay=args.dir_decay,
                nbr_file=args.nbr_file,
            ...
            albert_vocab = get_albert_vocab(args)
            hull = DecayAlphaHull.build(
                alpha=args.dir_alpha,
                decay=args.dir_decay,
                nbr_file=args.nbr_file,
            ...
            vocab = VOCAB[args.model_type](args)
    ...
    def _build_raw_dataset(self, args: ProgramArgs):
        ...
        if args.use_dev_aug == 'False':
            ...
            eval_raw = self.dataset_reader.read_from_file(file_path=f"{args.workspace}/dataset/{args.dataset_name}",
                                                       split=f"dev_{args.dev_aug_attacker}_{args.dev_aug_ratio}", args=args)
    ...
    def _build_attacker(self, args: ProgramArgs):
        #
        # if args.training_type in ['ours']:
        #     model_wrapper = HuggingFaceModelWrapperOurs(self.model, self.tokenizer, batch_size=args.batch_size)
        # el
        if args.training_type in ['dne', 'safer', 'mask']:
           model_wrapper = HuggingFaceModelEnsembleWrapper(args, self.model, self.tokenizer)
        elif args.model_type != 'lstm':
            model_wrapper = HuggingFaceModelWrapper(args,self.model, self.tokenizer, batch_size=args.batch_size)
        else:
...
if __name__ == '__main__':
    ...
    if args.dataset_name in ['agnews', 'sst-2']:
        args.max_seq_len = 128
    if args.training_type in ['tmd', 'none']:
        if args.dataset_name == 'agnews':
            args.gm_path = "/workspace/IGD/submission_to_github/baselines/tmd-main/outputs/infogan_bert_ag_news/none/82xcitkx/checkpoints/epoch=99-step=10599.ckpt"  # best
            args.model_name_or_path = "/workspace/IGD/submission_to_github/baselines/tmd-main/models/bert-base-uncased-ag_news"
        elif args.dataset_name == 'yelppolarity':
            args.model_name_or_path="/workspace/IGD/baselines/tmd-main/models/bert-base-uncased-yelp_polarity"
            args.gm_path = "/workspace/IGD/baselines/tmd-main/outputs/infogan_bert_yelp_polarity_2500/uncategorized/hg6da1jy/checkpoints/epoch=99-step=2199.ckpt"
            args.gm_path = "/workspace/IGD/baselines/tmd-main/outputs/infogan_bert_yelp_polarity/none/msx7aywh/checkpoints/epoch=99-step=2199.ckpt"
        elif args.dataset_name == 'imdb':
            args.model_name_or_path="/workspace/IGD/baselines/tmd-main/models/bert-base-uncased-imdb"
            args.gm_path =  "/workspace/IGD/submission_to_github/baselines/tmd-main/outputs/infogan_bert_imdb/none/3f2ps7zq/checkpoints/epoch=99-step=2199.ckpt"
        elif args.dataset_name == 'sst-2':
            args.model_name_or_path = "/workspace/IGD/submission_to_github/baselines/tmd-main/models/roberta-base-uncased-sst2/"
            args.gm_path = "/workspace/IGD/submission_to_github/baselines/tmd-main/outputs/infogan_roberta_sst2/none/55paajlv/checkpoints/epoch=99-step=5999.ckpt"


    args.safer_perturbation_set = os.path.join('{}/dataset/embedding/{}_{}'.format(args.workspace,args.dataset_name, args.safer_perturbation_set))
    args.dataset_path= args.workspace+'dataset/'+args.dataset_name
