...
class HuggingFaceModelWrapperOurs(PyTorchModelWrapper):
    ...
    def __call__(self, text_input_list):
        ...
        outputs = self.tokenizer.batch_encode_plus(text_input_list,
                                                   max_length=max_length, truncation='only_first',
                                                   padding='max_length',
                                                   return_token_type_ids=True, return_tensors='pt')

        eval_dataset = TensorDataset(outputs['input_ids'], outputs['token_type_ids'], outputs['attention_mask'])
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=self.batch_size)
        ret = []
        for batch in eval_dataloader:
            self.model.eval()
            batch = [t.to(model_device) for t in batch]
            # print(len(text), batch[0].shape, batch[0].device)
            with torch.no_grad():
                # logits = self.model([input_ids=batch[0], token_type_ids=batch[1], attention_mask=batch[2]])
                logits = self.model(batch)[0]
                #     'pooler_output']
                #
                # # logits = self.cosine_sim(outputs, self.weight)
                # logits = self.cosine_sim(outputs.repeat(1, 2), self.weight)
